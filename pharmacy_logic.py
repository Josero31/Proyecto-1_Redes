"""
Pharmacy MCP — shared business logic and tool registry.

Both transports (server.py = local stdio, remote_server.py = remote HTTP)
import this module so the two servers expose the exact same tools, data,
and behavior. Only the JSON-RPC transport layer differs between them.
"""
...
# ---------------------------------------------------------------------------
# JSON-RPC 2.0 message handling (transport-agnostic)
# ---------------------------------------------------------------------------

JSONRPC_VERSION = "2.0"
MCP_PROTOCOL_VERSION = "2025-06-18"
SERVER_NAME = "pharmacy-chatbot-server"
...
def process_message(msg):
    """Process one already-parsed JSON-RPC message dict and return a
    response dict, or None if no response should be sent (notifications)."""
...
def process_message_raw(raw_line):
    """Parse a raw JSON text line and delegate to process_message.
    Used by transports that read newline-delimited JSON (stdio)."""
    import json
